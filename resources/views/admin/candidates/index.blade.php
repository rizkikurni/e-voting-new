...
@section('content')
    <main class="page-content">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            @if ($event->isEditable())
                <a href="{{ route('events.candidates.create', $event->id) }}" class="btn btn-primary btn-sm radius-30">
        ...
        <div class="card">
            <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($candidates as $candidate)
                                <tr>
                                    ...
                                    <td>
                                        @if ($candidate->photo)
                                            <img src="{{ asset('storage/' . $candidate->photo) }}" class="rounded mb-3"
                                                width="80">
                                        @else
                                            <p>Tidak ada foto</p>
                                        @endif
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            @if ($event->isEditable())
                                                ...
                                                <form
                                                    action="{{ route('events.candidates.destroy', [$event->id, $candidate->id]) }}"
                                                    method="POST" onsubmit="return confirm('Hapus kandidat ini?')">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <a href="{{ route('candidates.index') }}" class="btn btn-secondary btn-sm mt-3">
